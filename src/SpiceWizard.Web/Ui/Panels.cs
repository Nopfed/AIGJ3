...
namespace SpiceWizard.Web.Ui
{
    ...
    public static class Panels
    {
        ...
        static void Board(Ui ui, GameState s, Session ss)
        {
            ...
            var q = s.Quota;
            ui.Label(Left, Top, "The town council requests, by the end of day " + s.Clock.Week * 7 + ":", Palette.DarkRed);
            if (q == null) { ui.Label(Left, Top + 14, "Nothing posted."); return; }
            for (int i = 0; i < q.Lines.Count; i++)
            {
                var line = q.Lines[i];
                var r = RecipeBook.Get(line.RecipeId);
                int y = Top + 16 + i * (Row + 2);
            ...
            int by = Top + 16 + q.Lines.Count * (Row + 2) + 8;
            ui.Label(Left, by, "Today is day " + s.Clock.DayOfWeek + " of 7.");
            ui.Label(Left, by + 12, "Reward: " + Balance.QuotaBonusPeppercorns(q.Week) + " peppercorns, " + Balance.QuotaBonusXp(q.Week) + " fame and a rare spice.");
            ui.Label(Left, by + 24, "Sauces on the list earn an extra star when sold.", Palette.Grey);
            ui.Label(Left, by + 34, "The town tires of the same sauce twice in three days.", Palette.Grey);
            if (q.IsMet) ui.Label(Left, by + 50, "Quota met! The bonus arrives at the start of next week.", Palette.Green);
        }
        ...
        static void Market(Ui ui, GameState s, Session ss)
        {
            ...
            ui.Label(Left, Top, "Seeds", Palette.DarkRed);
            for (int k = 0; k < Species.All.Length; k++)
            {
                var info = Species.All[k];
                int y = Top + 12 + k * Row;
                bool unlocked = info.UnlockLevel <= s.Level;
                ui.IconLabel(Left, y, ItemArt.PepperIcon(info.Species), unlocked ? Color.White : Palette.Grey, info.Name, unlocked ? Palette.Outline : Palette.Grey);
                ui.Label(Left + 56, y + 1, info.SeedCost + "pc", Palette.Outline);
                if (!unlocked) ui.Label(Left + 86, y + 1, "Lv" + info.UnlockLevel, Palette.Grey);
                else if (ui.Button(new Rectangle(Left + 84, y - 1, 30, 11), "Buy", s.Peppercorns >= info.SeedCost, info.CareHint))
                    ss.Say(Actions.BuySeed(s, info.Species));
                ui.Label(Left + 120, y + 1, "x" + s.Inventory.Seed(info.Species), Palette.Grey);
            }

            int sx = Left + 156;
            ui.Label(sx, Top, "Spices", Palette.DarkRed);
            for (int k = 0; k < Inventory.SpiceCount; k++)
            {
                var spice = (Spice)k;
                int y = Top + 12 + k * Row;
                bool unlocked = SpiceInfo.UnlockLevel(spice) <= s.Level;
                ui.IconLabel(sx, y, "ic_pouch", unlocked ? ItemArt.SpiceColor(spice) : Palette.Grey, SpiceInfo.Name(spice), unlocked ? Palette.Outline : Palette.Grey);
                ui.Label(sx + 84, y + 1, SpiceInfo.Price(spice) + "pc", Palette.Outline);
                if (!unlocked) ui.Label(sx + 112, y + 1, "Lv" + SpiceInfo.UnlockLevel(spice), Palette.Grey);
                else if (ui.Button(new Rectangle(sx + 110, y - 1, 30, 11), "Buy", s.Peppercorns >= SpiceInfo.Price(spice)))
                    ss.Say(Actions.BuySpice(s, spice));
                ui.Label(sx + 146, y + 1, "x" + s.Inventory.SpiceOf(spice), Palette.Grey);
            }
            ui.Paragraph(Left, Top + 12 + 4 * Row + 8, 24, "\"Peppercorns are money AND an ingredient. Grind your own pepper powder at the mortar.\"", Palette.Grey);
        }
        ...
        static void Cauldron(Ui ui, GameState s, Session ss)
        {
            ...
            for (int k = 0; k < RecipeBook.All.Length; k++)
            {
                var r = RecipeBook.All[k];
                int y = Top + k * 12;
                bool unlocked = r.UnlockLevel <= s.Level;
                var rowRect = new Rectangle(Left, y - 1, 162, 11);
                bool selected = ss.SelectedRecipe == r.Id;
                if (selected) ui.C.Rect(rowRect, Palette.Yellow * 0.5f);
                ui.IconLabel(Left + 2, y, ItemArt.SauceIcon(r), unlocked ? ItemArt.SauceColor(r.Id) : Palette.Grey, r.Name, unlocked ? Palette.Outline : Palette.Grey);
                ui.Label(Left + 100, y + 1, unlocked ? r.BaseValue + "pc" : "Lv" + r.UnlockLevel, unlocked ? Palette.Outline : Palette.Grey);
                if (ui.Take(rowRect)) ss.SelectedRecipe = r.Id;
                string blocker = Actions.CookBlocker(s, r, ss.ExtraPeppercorn);
                if (unlocked && ui.Button(new Rectangle(Left + 130, y - 1, 34, 11), "Cook", blocker.Length == 0, blocker.Length == 0 ? "Costs " + Balance.CookSpiceCost + " spice" : blocker))
                {
        ...
        static void Shelf(Ui ui, GameState s, Session ss)
        {
            ...
            for (int j = 0; j < FermentShelf.MaxJars; j++)
            {
                int y = Top + 14 + j * 34;
                ...
                {
                    ui.Label(Left + 16, y + 17, "Bubbling away...", Palette.Grey);
                }
            }
        }
        ...
        static void Crate(Ui ui, GameState s, Session ss)
        {
            ...
            ui.Label(Left, Top, "Bottled (" + s.Inventory.Sauces.Count + ")", Palette.DarkRed);
            if (s.Inventory.Sauces.Count == 0) ui.Label(Left, Top + 12, "Nothing to ship. Cook first!", Palette.Grey);
            for (int k = 0; k < s.Inventory.Sauces.Count && k < 10; k++)
            {
            ...
            int cx = Left + 160;
            ui.Label(cx, Top, "In the crate (" + s.Crate.Sauces.Count + "/" + Balance.CrateCapacity + ")", Palette.DarkRed);
            if (s.Crate.Sauces.Count == 0) ui.Label(cx, Top + 12, "Empty. The cart leaves at nightfall.", Palette.Grey);
            for (int k = 0; k < s.Crate.Sauces.Count; k++)
            {
