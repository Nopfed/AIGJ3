...
public class BotPlaythroughTests
{
    ...
    public void Greedy_bot_masters_the_craft_in_a_sensible_number_of_days()
    {
        ...
        for (ulong seed = 1; seed <= Seeds; seed++)
        {
            ...
            days.Add(day);
            _out.WriteLine($"seed {seed}: master on day {day}, {bot.State.Stats.SaucesSold} sauces, {bot.State.Stats.PeppercornsEarned} pc earned, {bot.State.Stats.QuotasMet} quotas");
        }
        days.Sort();
